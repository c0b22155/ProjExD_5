...
* 爆弾、壁、プレイヤーの当たり判定（担当：若林）：爆弾の爆発範囲に、破壊可能な壁もしくはプレイヤーがいた場合、破壊する。二つのオブジェクトの座標が同一かどうかを判定するクラスを作成し、同一だった場合、破壊する。

* 破壊可能な壁を設置する機能（担当：喜多村）：初期マップにて、プレイヤーが通行可能なマスに破壊可能な壁をランダムに配置する機能。

* 無敵機能（担当：高尾）：ゲーム中にキャラクターが一度だけ使用可能な機能。使用時間中は爆弾の判定を無効にする。"i"を押すと無敵化する。
